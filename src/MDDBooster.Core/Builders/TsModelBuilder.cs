...
";
    ...
    private void WriteClass(IClassTypeUnit model, StringBuilder sb, List<string> usingLines, bool isInterface = false, bool isAbstracts = false)
    {
        ...
        var interfaces = string.Join(",", model.GetInterfaces()
            .Select(p => ResolveTypeNames(p, usingLines))
            .Where(p => string.IsNullOrEmpty(p) != true));
                ...
                if (propertyMetaTypeMap.TryGetValue(type, out string? propertyMetaType))
                {
                    ...
                    if (property.Type == "string")
                    {
                        if (property.Name.Contains("email", StringComparison.OrdinalIgnoreCase))
                            ...
                            pType = "url";
                        else if (property.Name.Contains("tel", StringComparison.OrdinalIgnoreCase))
                            pType = "tel";
                        else if (property.Name.EndsWith("area", StringComparison.OrdinalIgnoreCase) ||
                    ...
                    if (property.IsRequired)
                        additionalProps.Add("required: true");

                    // 길이 제한이 있는 경우
                    if (property.HasMaxLength(out var maxLength) && maxLength > 0)
                    {
                        additionalProps.Add($"length: {maxLength}");
                    }

                    // 최소/최대값이 있는 경우 (숫자 타입)
                    if (pType == "number")
                    {
                        if (property.HasRange(out var minValue, out var maxValue))
                        {
                            if (minValue != null)
                                additionalProps.Add($"min: {minValue}");
                            if (maxValue != null)
                                additionalProps.Add($"max: {maxValue}");
                        }
                    }

                    // 패턴이 있는 경우
                    if (property.HasRegularExpression(out var pattern) && !string.IsNullOrEmpty(pattern))
                    {
                        additionalProps.Add($"pattern: \"{pattern.Replace("\"", "\\\"")}\"");
                    }

                    // 파일 타입인 경우
                    if (pType == "file" && property.HasFileAccepts(out var accepts) && accepts.Length > 0)
                    {
                        additionalProps.Add($"accepts: [{string.Join(", ", accepts.Select(a => $"\"{a}\""))}]");
                    }

                    // 레이블 가져오기 (Display 특성이 있으면 사용하고, 없으면 추천 레이블 사용)
                    var label = property.GetLabel();
                    if (string.IsNullOrEmpty(label) || label == property.Name)
                    {
                        label = property.GetSuggestedLabel();
                    }

                    var additionalPropsText = additionalProps.Count > 0 ? ", " + string.Join(", ", additionalProps) : "";
                    propertyMetaDecorator = $@"@propertyMeta({{ type: ""{pType}"", label: '{label}'{additionalPropsText} }})";
                }
            }
    ...
    {
        // 콤마로 구분된 모든 타입 이름 처리
        var typeNames = typeLine.Split(',').Select(t => t.Trim()).Where(t => !string.IsNullOrEmpty(t));
        ...
        foreach (var tName in typeNames)
        {
            if (tName.Contains('<')) // is generic
            {
                ...
                if (!string.IsNullOrEmpty(resolved)) sb.Append(resolved);

                var list = new List<string>();
                foreach (var parameter in parameters)
                {
                    resolved = ResolveTypeNames(parameter, usingLines);
                    if (!string.IsNullOrEmpty(resolved)) list.Add(resolved);
                }
                if (list.Count > 0)
                {
                    sb.Append('<');
                ...
                var resolvedType = sb.ToString();
                if (!string.IsNullOrEmpty(resolvedType))
                {
                    resolvedNames.Add(resolvedType);
                }
            }
            else
            {
                var typeName = tName.Trim();
                if (typeName.Equals("IEnumerable"))
                {
                    resolvedNames.Add("Array");
                }
                else if (this.interfaces.Any(p => p.Name == typeName)
                    || this.abstracts.Any(p => p.Name == typeName)
                    || this.classes.Any(p => p.Name == typeName))
                {
                    // 같은 파일에 있어서 using을 추가할 필요가 없습니다.
                    resolvedNames.Add(typeName);
                }
                else
                {
                    var m = FindModel(typeName);
                    if (m.Item1 != null)
                    {
                        resolvedNames.Add($"{m.Item1.ns}.{typeName}");
                        TryAddUsingLInes(m, usingLines);
                    }
                    // m.Item1이 null인 경우는 타입을 찾을 수 없는 경우이므로 무시합니다.
                }
            }
        }
